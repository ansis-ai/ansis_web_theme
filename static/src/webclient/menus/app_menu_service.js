                    ...
                    getAppsMenuItems() {
                        ...
                        if (Array.isArray(savedOrder) && savedOrder.length) {
                            apps.sort((a, b) => {
                                ...
                                if (aIdx === -1) aIdx = savedOrder.indexOf(a.id);
                                if (aIdx === -1) aIdx = savedOrder.indexOf(String(a.id));

                                let bIdx = savedOrder.indexOf(b.xmlid);
                                if (bIdx === -1) bIdx = savedOrder.indexOf(b.id);
                                if (bIdx === -1) bIdx = savedOrder.indexOf(String(b.id));
